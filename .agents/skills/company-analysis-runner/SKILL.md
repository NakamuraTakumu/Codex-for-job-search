...
# Main workflow
1. If multiple companies are given, first inspect each company’s official information and collect major subsidiaries or nearby entities across the whole set.
...
8. For each fixed track, launch one child agent using the `company-analysis` evaluator. Do not block on file naming or review payload details before starting the child.
...
9. Instruct each child to return a complete YAML object only.
10. Instruct the child to first fill `fact_layer.official` and `facts_official` from official information, then add `fact_layer.unofficial` and `facts_unofficial` as supporting unofficial information, and only then assign a single final `score`.
11. If important gaps remain after the official pass, require the child to perform at least one unofficial pass. Even if no usable value is found, require it to record searched unofficial lineages and failed-search reasons.
12. Do not require full official completeness before unofficial observations can be recorded. Relevant unofficial observations may be kept in `facts_unofficial` or `summary.concerns` as soon as they are found.
13. Explicitly tell the child not to mix official and unofficial information in the same field.
14. Explicitly tell the child to count unofficial evidence by independent lineage rather than raw URL count, and not to treat reposts, mirrors, or alternate surfaces of the same service as independent support.
15. If needed, explicitly tell the child not to read existing reports or other agent outputs.
16. Use one child per target as the default, and never feed `research` and `swe` sequentially to the same child.
17. Make the fixed `evaluation_target`, `hiring_entity`, and `job_type` explicit in the prompt so the child does not silently switch targets.
18. The parent chooses the `slug` for each target. Default to the shortest stable target-based slug that remains unambiguous, for example `japan_ibm_research` or `ntt_data_swe`.
...
19. Append `run_metadata` to the YAML before accepting it as final. At minimum, use `executor`, `model`, `reasoning_effort`, and `fixed_by_parent`.
20. The parent decides where the returned YAML will be saved.
21. Run `python3 tool/check_company_analysis_yaml.py <yaml-file>` to validate the format.
22. If validator checks fail, list the schema violations and send the YAML back to the child for a full re-output.
23. Once validator checks pass, the parent performs a light high-risk review on the YAML itself and decides whether a content review is needed before rendering.
24. Only when that pre-render check suggests content review is needed, embed the fixed scope and analysis YAML directly into the prompt and send them to one shared review child to obtain review YAML.
   - As a rule, specify `gpt-5.4-mini` for the review child as well.
25. Run `python3 tool/check_company_analysis_review.py <review-yaml>` to validate the review schema.
26. If the content-review verdict is `revise`, list the findings and send them back to the analysis child, requiring a full corrected YAML re-output.
27. Once the YAML is accepted for rendering, generate Markdown with `python3 tool/render_company_analysis_md.py <yaml-file>`.
28. Only when render-level confirmation is needed should the parent send the rendered Markdown, together with the fixed scope and analysis YAML, to the review child for a render-focused review.
29. Run `python3 tool/check_company_analysis_review.py <review-yaml>` for that render-focused review as well.
30. If the render-focused review returns `revise`, resolve the findings, rerun renderer or reviewer as needed, and only then finalize the artifacts.
31. In the final answer, refer to the saved YAML and Markdown, and refer to the review artifact as well if review was generated.
